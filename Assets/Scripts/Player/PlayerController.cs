...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("The trandform of the camera following the player")]
    private Transform m_CameraTransform;

    [SerializeField]
    [Tooltip("A list of all attacks and information about them")]
    private PlayAttackInfo[] m_Attacks;

    [SerializeField]
    [Tooltip("Amount of health the play starts with")]
    ...
    //In order to do anything, we cannot be frozen (timer bust be 0)
...
//            cc_Rb.rotation = Quaternion.Slerp(cc_Rb.rotation, cc_Rb.rotation * Quaternion.Euler(0, theta, 0), 0.2f);
    ...
    public void DecreaseHealth(float amount)
    {
        ...
        m_HUD.UpdateHealth(1.0f * p_CurrentHealth / m_MaxHealth);
    ...
    {
        p_CurrentHealth += amount;
        if(p_CurrentHealth > m_MaxHealth)
        {
            p_CurrentHealth = m_MaxHealth;
        }
        m_HUD.UpdateHealth(1.0f * p_CurrentHealth / m_MaxHealth);
